...
function cards() {
  obj.forEach( function(item, key)  {
    ...
    div.innerHTML += `
    <div data-aos="fade-up" class="card" style=" width: 320px; height: auto; padding: 5px 10px;">
      <center>
        <img src="${item.images[0]}" alt="" style="width: 250px;">
        <div class="name"><b>Name:</b> ${item.title}</div>
        <div class="category"><b>Category</b> : ${item.category}</div>
        <div class="price"><b>Price:</b> $${item.price} <span> <b>Rate: </b> ${item.rating}</span> <div>
        <button onclick="add_to_cart(${key})" class="btn1 btn-outline-success">Add to cart</button>
    </div>
    `
  ...
  let div = document.createElement("div");
    ...
    <div  class="card" style=" width: 320px; height: 60vh; padding: 5px 10px;">
      <center>
        ...
        <div class="name"><b>Name:</b> ${item.title}</div>
        <div class="category"><b>Category</b> : ${item.category}</div>
        <div class="price"><b>Price:</b> $${item.price} <span>Rate: ${item.rating}</span> <div>
        <button onclick="add_to_cart(${key})" class="btn1 btn-outline-success">Add to cart</button>
    </div>
    `
    main.appendChild(div)
  if (!uniquecategory.includes(item.category)) {
    select.innerHTML += `<option value="${item.category}">${item.category}</option>`;
...
select.onchange =  function() {
  ...
  obj.forEach(function(i , key){
    ...
    <div class="card" style=" width: 320px; height: 60vh; padding: 5px 10px;">
      <center>
        <img src="${i.images[0]}" alt="" style="width: 250px;">
        <div class="name"><b>Name:</b> ${i.title}</div>
        <div class="category"><b>Category</b> : ${i.category}</div>
        <div class="price"><b>Price:</b> $${i.price} <span>Rate: ${i.rating}</span> <div>
        <button onclick="add_to_cart(${key})" class="btn1 btn-outline-success">Add to cart</button>
    </div>
    `
    main.appendChild(div);
    ...
    if (title.includes(searchValue)) {
      ...
      <div class="card" style=" width: 320px; height: 60vh; padding: 5px 10px;">
        <center>
          ...
          <div class="category"><b>Category</b> : ${i.category}</div>
          <div class="price"><b>Price:</b> $${i.price} <span>Rate: ${i.rating}</span> <div>
          <button onclick="add_to_cart(${key})" class="btn1 ">Add to cart</button>
      </div>
      `
      main.appendChild(div);
    }
...
function add_to_cart(key) {
  ...
  // console.log(obj[Id])
  if (checkoutlist[key] == null) {
    checkoutlist[key] = obj[key]
...
function relodCart() {
  ...
  checkoutlist.forEach((item, key) => {
    ...
    <img src="${item.images}" alt="">
        ...
        <div class="name1">${item.category}<br>$${item.price}</div>
        </div>
        <div>
        <button id="add1" onclick="add2(${key},${item.quantity -  1})">-</button>
        <span id="Zero2">${item.quantity}</span>
        <button id="add1" onclick="add2(${key},${item.quantity + 1})">+</button>
        </div>
    `
    productList.appendChild(li)
  });
  total.innerHTML = `<small>Total(${count} item) $</small>`+ totalPrice;
  quantity.innerHTML = count;
}

function add2(key, quantity){
  if(quantity == 0){
